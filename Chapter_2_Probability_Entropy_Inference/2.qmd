---
title: 2 — Probability, Entropy, and Inference
execute:
  echo: false
format:
  revealjs:
...
An **ensemble** $X$ is a framework for a random variable, defined by a set of possible outcomes, or alphabet, $A_X = \{a_1, a_2, ..., a_I\}$, and their corresponding probabilities, $P_X = \{p_1, p_2, ..., p_I\}$. These probabilities must be non-negative ($p_i \ge 0$) and sum to unity ($\sum P(x=a_i) = 1$). The probability of a subset of outcomes $T$ is the sum of the probabilities of all outcomes within that subset.
...
##
Using $B(x)$ to denote the "degree of belief in proposition $x$" and $\overline{x}$ for its negation, the axioms are:

* **Axiom 1: Ordering**
    Degrees of belief can be ordered. If the belief in $x$ is greater than the belief in $y$, and the belief in $y$ is greater than the belief in $z$, then the belief in $x$ must be greater than the belief in $z$. This allows beliefs to be mapped onto real numbers.

* **Axiom 2: Negation**
    The degree of belief in a proposition $x$ and its negation $\overline{x}$ are related. There is a function $f$ such that $B(x) = f[B(\overline{x})]$.

* **Axiom 3: Conjunction**
    The degree of belief in a conjunction of propositions ($x$ AND $y$) is related to the belief in the conditional proposition ($x$ given $y$) and the belief in $y$. There is a function $g$ such that $B(x,y) = g[B(x|y), B(y)]$.

If a set of beliefs satisfies these axioms, they can be represented by probabilities $P(x)$ that obey the standard rules, such as $P(x) = 1 - P(\overline{x})$ and the product rule $P(x,y) = P(x|y)P(y)$.

---

## 2.3 Forward and Inverse Probabilities .smaller
